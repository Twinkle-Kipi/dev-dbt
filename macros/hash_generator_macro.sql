{% macro caribou_to_ods_layer_macro(src_db,src_schema,table_name) %}
{% set column_lst %}
...
{% set final_stmt %}
        ...
        {% if is_incremental() %}
            WHERE ETL_UPDATEDDATE >= (select coalesce(MAX(ETL_UPDATEDDATE),to_date('1900-01-01 00:00:00')) from {{ this }})
        {% endif %} 
{% endset %}
{{ return( final_stmt ) }}
{% endmacro %}
